...
**Constraints**: <100 lines per file (constitution), minimal dependencies, handle files up to memory limits via streaming
...
**100-Line Rule Check**:
- [x] No single file planned exceeds 100 lines (each module <100 lines: reader, progress, CLI, exceptions)
...
**Structure Decision**: Single Python module structure chosen. The `pdf_extractor/` directory contains all core functionality split into focused modules (each <100 lines per constitution). CLI interface accessible via `python -m pdf_extractor`. Tests organized by type (contract, integration, unit) following TDD approach.
...
- Group by constitutional file size limits (<100 lines each)
...
- [x] Post-Design Constitution Check: PASS (all modules <100 lines, composition-first design, minimal dependencies)
