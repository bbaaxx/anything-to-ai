...
from anything_to_ai.pdf_extractor.models import (
    ExtractionConfig,
    ExtractionResult,
    PageResult,
)
...
class EnhancedExtractionConfig(ExtractionConfig):
    ...
    image_processing_config: object | None = None
...
class ImageExtractionInterface(ABC):
    ...
    @abstractmethod
    def crop_image_from_page(self, page_number: int, file_path: str, bounding_box: tuple) -> object:
...
def validate_enhanced_extraction_config(config: EnhancedExtractionConfig) -> bool:
    ...
    return (
        ...
        and (config.max_images_per_page is None or isinstance(config.max_images_per_page, int))
        and isinstance(config.image_batch_size, int)
        and config.image_batch_size >= 1
        and config.image_batch_size <= 10
