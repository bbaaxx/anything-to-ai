...
class CLIProgressReporterInterface(ABC):
    ...
    @abstractmethod
    def report_page_progress(self, page_number: int, total_pages: int, images_found: int = 0) -> None:
    ...
    @abstractmethod
    def report_image_progress(self, image_number: int, total_images: int, status: str) -> None:
    ...
    @abstractmethod
    def report_completion(self, total_pages: int, total_images: int, processing_time: float) -> None:
...
def validate_cli_parser(parser: argparse.ArgumentParser) -> bool:
    ...
    optional_args = {"include_images", "image_style", "image_fallback", "max_images", "batch_size", "format", "output", "progress"}

    return required_args.issubset(actions) and optional_args.issubset(actions)


def validate_parsed_args(args: argparse.Namespace) -> bool:
    """Validate parsed arguments contract."""
    required_attrs = ["file_path"]
    optional_attrs = ["include_images", "image_style", "image_fallback", "max_images", "batch_size", "format", "output", "progress"]

    return all(hasattr(args, attr) for attr in required_attrs) and all(hasattr(args, attr) for attr in optional_attrs)


def validate_extraction_result_format(result: dict[str, Any]) -> bool:
    """Validate extraction result format contract."""
    required_keys = {"text", "pages", "total_pages", "processing_time"}
    optional_keys = {"enhanced_text", "total_images_found", "total_images_processed", "total_images_failed", "vision_model_used", "image_processing_time"}

    return isinstance(result, dict) and required_keys.issubset(result.keys()) and all(key in required_keys.union(optional_keys) for key in result)
...
CLI_ARGUMENT_SPECS = {
    "file_path": {"type": str, "help": "Path to PDF file to process", "required": True},
    "include_images": {"action": "store_true", "help": "Include AI-generated descriptions of images found in PDF", "default": False},
    "image_style": {"choices": ["brief", "detailed", "technical"], "default": "detailed", "help": "Style of image descriptions"},
    "image_fallback": {"type": str, "default": "[Image: processing failed]", "help": "Fallback text when image processing fails"},
    "max_images": {"type": int, "help": "Maximum number of images to process per page"},
    "batch_size": {"type": int, "default": 4, "help": "Batch size for image processing (1-10)"},
    "format": {"choices": ["text", "json", "csv"], "default": "text", "help": "Output format"},
    "output": {"type": str, "help": "Output file path (default: stdout)"},
    "progress": {"action": "store_true", "help": "Show progress information", "default": False},
}
