...
class PDFImageProcessor:
        ...
        if config.include_images:
            ...
            if not vision_model:
                raise VLMConfigurationError(
                    "VISION_MODEL environment variable required for image processing",
    ...
    def extract_with_images(self, file_path: str, config: EnhancedExtractionConfig) -> EnhancedExtractionResult:
        ...
        try:
            with pdfplumber.open(file_path) as pdf:
                ...
                regular_pages = [PageResult(
                    page_number=ep.page_number,
                    text=ep.text,
                    char_count=ep.char_count,
                    extraction_time=ep.extraction_time
    ...
    def _process_page(self, page, page_num: int, file_path: str, config: EnhancedExtractionConfig) -> EnhancedPageResult:
        ...
        enhanced_page = EnhancedPageResult(
        ...
        if config.include_images:
            ...
            if self.circuit_breaker.can_process() and self.image_processor and image_contexts:
                processed_count, failed_count = self._process_images(
                    image_contexts, page_num, file_path, config
                )
    ...
    def _process_images(self, image_contexts: List[ImageContext], page_num: int, file_path: str, config: EnhancedExtractionConfig):
        ...
        for context in image_contexts:
            try:
                ...
                pil_image = self.image_extractor.crop_image_from_page(
                    page_num, file_path, context.bounding_box
                )
