    ...
    for command in commands:
        ...
        if result.returncode == 0:
            assert "usage:" in result.stdout.lower() or "usage:" in result.stderr.lower(), \
                f"{command} should show usage information"
        else:
            # Command not found is expected initially
            assert "not found" in result.stderr.lower() or "command not found" in result.stderr.lower() or \
                   result.returncode != 0, f"{command} should either work or not be installed yet"
...
def test_cli_modules_importable():
    ...
    for module in modules:
        try:
            ...
            assert "No module named" in str(e) or "anyfile_to_ai" in str(e), \
                f"Import error should be about missing package: {e}"
...
def test_cli_entry_point_format():
    ...
    lines = [line.strip() for line in content.split('\n') 
             ...
             and any(cmd in line for cmd in ['pdf-extractor', 'image-processor', 'audio-processor', 'text-summarizer'])]
